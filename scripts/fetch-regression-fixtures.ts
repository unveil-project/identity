...
async function fetchUserEvents(username: string): Promise<unknown[]> {
...
async function fetchFixture(username: string): Promise<void> {
  try {
    ...
    console.log(`✅ ${username}: ${events.length} events, ${user.public_repos} repos`);
  } catch (error) {
    console.error(`❌ ${username}: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
}
...
async function main(): Promise<void> {
  ...
  if (!GITHUB_TOKEN) {
    console.log("⚠️  No GITHUB_TOKEN set - using unauthenticated API (60 req/hr limit)");
